...
<body>
  ...
  if (empty($_SESSION["username"])) {
    ...
    <h2 style=" font-size: 50px;
     ...
     margin:90px; font-weight: bold;">Looks like You haven't login</h2>
    ...
    <div class="container" style=" margin-top:55px">
      ...
      if (isset($_POST["add"])) {
        ...
        $sq = "select quantity from cart where name= '$n'";
        $res = mysqli_query($conn, $sq);
        $avail = mysqli_fetch_assoc($res);

        if ($q <= $avail['quantity']) {

          $rzult = mysqli_query($conn, "select price from cart where name='$n'");

          $pri = mysqli_fetch_assoc($rzult);
          $price = $q * $pri['price'];
          $insrt = "insert into buyproduct values('$n','$q','$price')";

          $add = mysqli_query($conn, $insrt);
          echo '
...
<svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg>
   <strong> Product Quantity not available in Cart !!!</strong>
<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
</button>
</div>
...
';
  ...
  ';
    ...
    <form method="post">
      <div class="form-group">
        ...
        <input type="text" class="form-control" id="product_name" name="pn" placeholder="Enter product name" required>
      </div>

      <div class="form-group">
        <label for="quantity">Quantity</label>
        <input type="number" class="form-control" id="quantity" name="quanti" placeholder="Enter product quantity" required>
      </div>
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              <?php echo "<a class='cancel' style='font-size: 25px;' href='delete.php?val=" . $row['name'] . "'><i class='fa fa-trash del-icon'></i></a>"; ?>
  ...
  <div class="d-flex justify-content-center "><button type="submit" class="btn btn-primary" name="bill"
      value="Bill">Bill PDF</button>
